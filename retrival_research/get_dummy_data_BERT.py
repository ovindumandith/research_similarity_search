...
model_name = "bert-base-uncased"  # Or any other model you prefer
...
def get_bert_embedding(text):
    ...
    # Mean pooling (average of all token embeddings) for the sentence embedding
...
for idx, description in enumerate(descriptions):
    ...
    # Ensure the vector length is exactly 384 (pad/truncate if necessary)
    if len(embedding) < 384:
        embedding += [0.0] * (384 - len(embedding))  # Pad to 384 dimensions
    elif len(embedding) > 384:
        embedding = embedding[:384]  # Truncate to 384 dimensions if necessary
...
with open('dummy_data_for_milvus.json', 'w') as json_file:
    json.dump(description_vectors_list, json_file, indent=2)

print("Dummy data saved to 'dummy_data_for_milvus.json'.")
