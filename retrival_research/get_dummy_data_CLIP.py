...
# Load pre-trained CLIP model and processor (tokenizer and feature extractor combined)
model_name = "openai/clip-vit-base-patch32"  # CLIP model (vision transformer version)
...
# Function to generate CLIP embeddings
def get_clip_embedding(text):
    # Preprocess the text to match CLIP input format
    ...
    # CLIP text embeddings are already the right size, so no need to pad or truncate
...
for idx, description in enumerate(descriptions):
    ...
    embedding = get_clip_embedding(description)
    
    # CLIP embeddings typically have 512 dimensions, so we don't need to modify the size
    if len(embedding) != 512:
        print(f"Warning: The embedding size for description {idx} is {len(embedding)} instead of 512.")

    # Create a dictionary for each entry with an auto-generated id
    entry = {
...
with open('dummy_data_for_milvus_clip.json', 'w') as json_file:
    json.dump(description_vectors_list, json_file, indent=2)

print("Dummy data saved to 'dummy_data_for_milvus_clip.json'.")
